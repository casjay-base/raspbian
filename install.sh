...
#    IFISONLINE=$( timeout 0.2 ping -c1 8.8.8.8 &>/dev/null ; echo $? )
    ...
    #    exit 1
    #else
   if [ ! -d /usr/share/httpd/.git ]; then rm -Rf /usr/share/httpd ; git clone -q https://github.com/casjay-templates/default-web-assets /usr/share/httpd; fi
   if [ -d /usr/share/httpd ]; then cd /usr/share/httpd && git pull -q; cd ~ ; fi
...
###############################################################################################
    ...
    # Setup apache2
  if [ -d /usr/share/httpd/.git ]; then cd /usr/share/httpd && git pull -q; cd ~ ; else sudo rm -Rf /usr/share/httpd ; sudo git clone -q https://github.com/casjay-templates/default-web-assets /usr/share/httpd; fi
    ...
    #use  sudo GH=username bash -c "$(wget -qO - https://github.com/casjay-base/raspbian/raw/master/install.sh)"
    if [ ! -z $GH ]; then
